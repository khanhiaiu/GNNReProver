...
def run_worker(gpu_id: int, study_name: str, db_path: str, n_trials_per_worker: int, args):
    ...
    dataset = remove_edge_types_args(args, dataset)
...
def train_ensemble_member_worker(
    ...
    dataset_member = remove_edge_types_args(args, dataset_member)
...
def run_hyperparameter_search(args: argparse.Namespace):
    ...
    try:
        for i in range(n_jobs):
            ...
            p = mp.Process(target=run_worker, args=(gpu_id, args.study_name, args.db_path, n_worker_trials, args))
            p.start()
            processes.append(p)
...
def run_ensemble_training(args: argparse.Namespace):
    ...
    dataset_main = remove_edge_types_args(args, dataset_main)
    ...
    for i in range(0, args.n_ensemble, n_jobs):
        ...
        for model_idx_in_batch, model_idx_global in enumerate(batch_indices):
            ...
            p = mp.Process(
                ...
                args=(gpu_id, model_idx_global, final_config, model_path, args.use_ema, args.ema_decay, args)
            )
            p.start()
...
def keep_edge_type(edge_index: Tensor, edge_type: Tensor, types_to_keep: List[int]) -> Tuple[Tensor, Tensor]:
    mask = torch.zeros(edge_type.size(0), dtype=torch.bool, device=edge_type.device)
    for t in types_to_keep:
        mask |= (edge_type == t)
    return edge_index[:, mask], edge_type[mask]

def remove_edge_types_args(args: argparse.Namespace, dataset_main : LightweightGraphDataset):
    edge_type_to_keep = [0, 1, 2] 
    if args.use_local == False:
        edge_type_to_keep.remove(dataset_main.edge_types_map["local_dependency"]) #TODO: check if key correct
    if args.use_goal == False:
        edge_type_to_keep.remove(dataset_main.edge_types_map["goal_dependency"]) # TODO: check if key correct
    dataset_main.premise_edge_index, dataset_main.premise_edge_attr = keep_edge_type(
        dataset_main.premise_edge_index,
        dataset_main.premise_edge_attr,
        edge_type_to_keep
    )
    dataset_main.context_edge_index, dataset_main.context_edge_attr = keep_edge_type(
        dataset_main.context_edge_index,
        dataset_main.context_edge_attr,
        edge_type_to_keep
    )
    # ----- New: allow disabling whole graphs (ablation studies) -----
    # If the user requested to NOT use the premise/context graphs, replace
    # their edge indices and attributes with empty tensors. The rest of the
    # codebase expects tensors (possibly empty) rather than None.
    if args.use_premise_graph is False:
        print("[ABlation] Disabling premise graph (setting premise edges to empty).")
        dataset_main.premise_edge_index = torch.empty((2, 0), dtype=torch.long)
        # Keep dtype consistent if possible
        dataset_main.premise_edge_attr = torch.empty((0,), dtype=dataset_main.premise_edge_attr.dtype)
    
    if args.use_context_graph is False:
        print("[ABlation] Disabling context graph (setting context edges to empty).")
        dataset_main.context_edge_index = torch.empty((2, 0), dtype=torch.long)
        dataset_main.context_edge_attr = torch.empty((0,), dtype=dataset_main.context_edge_attr.dtype)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-ensemble', type=int, default=1, help='Number of models in the ensemble.')
    ...
    parser.add_argument('--no-use-local', action='store_false', dest='use_local')

    parser.add_argument('--use-goal', action='store_true', default=True)
    parser.add_argument('--no-use-goal', action='store_false', dest='use_goal')

    parser.add_argument('--use-premise-graph', action='store_true', default=True)
    parser.add_argument('--no-use-premise-graph', action='store_false', dest='use_premise_graph')

    parser.add_argument('--use-context-graph', action='store_true', default=True)
    parser.add_argument('--no-use-context-graph', action='store_false', dest='use_context_graph')
